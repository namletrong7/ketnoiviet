<?php 
   ...
   // câu lệnh truy vấn lấy ra ảnh đại diện của gười thực hiện thông báo này 
	$chon = "SELECT * FROM users WHERE phonenumber = $phoneUser";
    ...
    $idNotifi .= $characters[rand(0, strlen($characters) - 1)];
     ...
     if (mysqli_num_rows($datachon) > 0) {
        while($row = mysqli_fetch_assoc($datachon)) {
            ...
            /// tạo thông báo
            $query = "INSERT INTO notifi VALUES ('$idNotifi','$idPost','$phoneUser','$avatar','$typeNotifi',null, '$timeNotifi')";
            $data = mysqli_query($conn, $query);

            // nó sẽ lấy ra tất cả những người quan tâm post này để tạo readPost 
            $humanCarePost = "SELECT phoneUser FROM carepost WHERE idPost = $idPost";
	        $dataHumanCarePost = mysqli_query($conn, $humanCarePost);
              ...
              while($row = mysqli_fetch_assoc($dataHumanCarePost)) {
               $phoneNumber= $row['phoneUser'];  // láy phone những người uqan tâm post đo 
               $query = "INSERT INTO readnotifi VALUES (null,'$idNotifi','$phoneNumber','false')";
               $data = mysqli_query($conn, $query);
              }
